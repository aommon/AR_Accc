...
public class nearby {
	...
	public static PointF[] nearbyLaLong (double s_la, double s_long,float degrees){
		//nearby
        PointF center = new PointF((float)s_la, (float)s_long);
        final double mult = 1.1; // mult = 1.1; is more reliable
        //double distance = 50;
        
        float ri = degree_right(degrees);
        
        float le = degree_left(degrees);
        
        PointF p1 = nearby.calculateDerivedPosition(center, mult * 70, 0); //right
        PointF p2 = nearby.calculateDerivedPosition(center, mult * 70, 90); //forward
        PointF p3 = nearby.calculateDerivedPosition(center, mult * 70, 180);//left
        PointF p4 = nearby.calculateDerivedPosition(center, mult * 70, 270);//back
